...
init(_Start_info) ->
  ...
  Names =
    [list_to_atom("store_vehicle_" ++ integer_to_list(X))
     || X <- lists:seq(0, env:svcount())],
  ChildSpecList =
    [generate_spec(rr_distributor, worker, store_vehicle_rr, Names)
     | [generate_spec(store_vehicle, worker, X, []) || X <- Names]],
