...
init(_Start_info) ->
  ...
  Names =
    [list_to_atom("get_facility_" ++ integer_to_list(X)) || X <- lists:seq(0, env:gfcount())],
  ChildSpecList =
    [generate_spec(rr_distributor, worker, get_facility_rr, Names)
     | [generate_spec(get_facility, worker, X, []) || X <- Names]],
